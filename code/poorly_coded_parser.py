...
class PoorlyCodedParser:
	...
	def parse_region(line: str, nano: nanoparticlebuilder.NanoparticleBuilder) -> Cylinder | None | Sphere | Plane | Cone | Prism:
		...
		if region_type == "cylinder":
			...
			assert PoorlyCodedParser.is_correct_parsing(line, shape.get_region(region_name)), f"Region {region_name} is not parsed correctly: \n{PoorlyCodedParser.split_command(shape.get_region(region_name))} != \n{line}"
	...
	def parse_set(line: str, nano: nanoparticlebuilder.NanoparticleBuilder) -> None:
		...
		if set_type == "region":
			...
			if prop == "type/subset":
				...
				assert PoorlyCodedParser.is_correct_parsing(line, result), f"Set type subset group {group_name} is not parsed correctly: \n{result} != \n{line}"
				logging.debug(result)
				return
			elif prop == "type/ratio":
				value = set_args[2]
				...
				assert PoorlyCodedParser.is_correct_parsing(line, result), f"Set type ratio group {group_name} is not parsed correctly: \n{result} != \n{line}"
				logging.debug(result)
				return
			else:
				raise ValueError(f"Unknown set property: {prop}")
	...
	def parse_group(line: str, nano: nanoparticlebuilder.NanoparticleBuilder) -> None:
		...
		if prop == "type":
			...
			assert PoorlyCodedParser.is_correct_parsing(line, result), f"Group type {group_name} is not parsed correctly: \n{result} != \n{line}"
			logging.debug(result)
			return
		elif prop == "region":
			region_name = line[3]
	...
	@staticmethod
	def parse_single_shape(shape_path: str, full_file: bool = False, replacements: dict | None = None) -> tuple[str, nanoparticlebuilder.NanoparticleBuilder]:
		"""
		...
		with open(shape_path, "r") as f:
			...
			lines = [template.TemplateUtils.replace_templates(ll.strip(), replacements) for line in lines if (ll := line.strip()) != ""]
			PoorlyCodedParser.parse_shape(lines, nano)
			return shape_path, nano

	@staticmethod
	def locate_relevant_lines(lines):
		start = PoorlyCodedParser.first_index_that_startswith(lines, "lattice") + 1
		try:
	...
	def first_index_that_startswith(lines: list[str], start: str) -> int:
		...
		assert len(out) > 0, f"Could not find line that starts with {start} - Perhaps you meant to use full_file = True?"
		return out[0]
