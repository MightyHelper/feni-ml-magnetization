...
class TokoUtils:
    ...
    def mkdir_toko(toko_path):
        return TokoUtils.run_cmd_for_toko(
            lambda user, toko_url: ["ssh", f"{user}@{toko_url}", f"mkdir", f"{toko_path}"])
    ...
    def wait_for_toko_execution(jobid):
        ...
        TokoUtils.run_cmd_for_toko(lambda user, toko_url: ["ssh", "-o", "ServerAliveInterval=10", f"{user}@{toko_url}",
                                                           f"sh -c 'while [ \"$({TOKO_SQUEUE} -hj {jobid})\" != \"\" ]; do sleep 1; done'"])
    ...
    def get_my_jobids() -> list[int]:
        ...
        :return:
        """
        return [int(jobid) for jobid in TokoUtils.run_cmd_for_toko(
            lambda user, toko_url: ["ssh", f"{user}@{toko_url}", f"sh -c '{TOKO_SQUEUE} -u {user} -h -o %i'"]).decode(
            "utf-8").split("\n") if jobid]
...
class TokoExecutionQueue(SingleExecutionQueue):
    ...
    def submit_toko_script(toko_nano_in: str, toko_sim_folder: str, local_sim_folder: str):
        ...
        return TokoUtils.run_cmd_for_toko(lambda user, toko_url: ["ssh", f"{user}@{toko_url}",
                                                                  f"sh -c 'cd {toko_sim_folder}; {config.TOKO_SBATCH} {config.SLURM_SH}'"])
    ...
    def simulate_in_toko(simulation_task: SimulationTask) -> str:
        ...
        sbatch_output = TokoExecutionQueue.submit_toko_script(toko_nano_in.as_posix(), toko_sim_folder.as_posix(),
