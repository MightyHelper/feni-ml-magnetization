...
def calc_complete(root_path, name):
    ...
    complete_df = pd.DataFrame([{
        'method': method,
        'rmse_fold': df.iloc[0]['RMSE_fold'],
        'rmse_full': df.iloc[0]['RMSE_full'],
        'r2_fold': df.iloc[0]['Rsquared_fold'],
        'r2_full': df.iloc[0]['Rsquared_full'],
    } for (method, df) in files])
    complete_df = complete_df.sort_values('method').reset_index(drop=True)
    complete_df['experiment'] = name
    print(complete_df.sort_values('rmse_fold'), end="\n\n")
...
print(all_experiments.sort_values('rmse_fold'))
